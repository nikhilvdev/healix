...
def _walk(root):
    return walk_frames(root, children_of=lambda n: n.children, url_of=lambda n: n.url, name_of=lambda n: n.name)
...
def test_paths_start_at_main_and_nest():
    tree = Node("http://a.com/", children=[Node("http://a.com/p", "panel", [Node("http://a.com/f", "form")])])
...
def test_unnamed_and_duplicate_siblings_get_unique_labels():
    tree = Node("http://a.com/", children=[Node("http://a.com/1"), Node("http://a.com/2", "x"), Node("http://a.com/3", "x")])
...
def test_cross_origin_frame_and_everything_below_it_is_flagged():
    tree = Node(
        "http://a.com/",
        children=[Node("http://b.com/", "foreign", [Node("http://a.com/back", "nested")]), Node("http://a.com/ok", "ok")],
    )
...
def test_about_blank_inherits_parent_origin():
    ...
    under_foreign = _walk(Node("http://a.com/", children=[Node("http://b.com/", "f", [Node("about:blank", "blank")])]))[2]
