...
class FakeDriver(Driver):
    ...
    def get_elements(self):
        ...
        anchors = [
            Element(tag="a", computed={"href": href}, attributes={"href": href}) for href in links
        ]
...
def test_scope_same_origin_is_stricter():
    pages = {
        "https://e.com/": (
            ["https://www.e.com/x", "http://e.com/plain", "https://e.com/ok"],
            "home",
        ),
        "https://e.com/ok": ([], "ok"),
    }
...
def test_urls_differing_only_by_noise_are_visited_once():
    pages = {
        "https://e.com/": (
            ["https://e.com/a?utm_source=x", "https://e.com/a/", "https://e.com/a#top"],
            "home",
        ),
        "https://e.com/a": ([], "a"),
    }
...
def product_site(n=6):
    pages = {
        "https://e.com/": (
            [f"https://e.com/product/{i}" for i in range(1, n + 1)] + ["https://e.com/about"],
            "home",
        )
    }
...
def test_same_template_pages_collapse_into_one_manifest_entry():
    ...
    assert sorted(urls(manifest)) == [
        "https://e.com/",
        "https://e.com/about",
        "https://e.com/product/1",
    ]
...
def test_url_only_dedupe_keeps_every_page():
    manifest, _ = crawl(
        product_site(), config=DiscoveryConfig(max_pages=100, dedupe_by="url_normalized")
    )
...
def test_deferred_urls_are_still_visited_when_budget_allows():
    ...
    assert sorted(driver.navigations) == sorted(
        [
            "https://e.com/",
            "https://e.com/about",
            *[f"https://e.com/product/{i}" for i in range(1, 7)],
        ]
    )
    assert driver.navigations.index("https://e.com/about") < driver.navigations.index(
        "https://e.com/product/3"
    )
...
def test_redirect_to_already_visited_page_is_not_recorded_twice():
    ...
    manifest, driver = crawl(
        pages,
        redirects={"https://e.com/old": "https://e.com/new"},
        config=DiscoveryConfig(dedupe_by="url_normalized"),
    )
...
def test_manifest_records_the_final_url_after_redirect():
    pages = {
        "https://e.com/": (["https://e.com/old"], "home"),
        "https://e.com/landing": ([], "landing"),
    }
...
def test_redirect_out_of_scope_is_skipped():
    pages = {
        "https://e.com/": (["https://e.com/sso"], "home"),
        "https://idp.com/login": ([], "idp"),
    }
...
def test_load_and_extraction_failures_are_recorded_and_the_crawl_continues():
    pages = {
        "https://e.com/": (
            ["https://e.com/down", "https://e.com/broken", "https://e.com/fine"],
            "home",
        ),
    ...
    manifest, _ = crawl(
        pages, broken={"https://e.com/down"}, extract_broken={"https://e.com/broken"}
    )
...
def test_classifier_sets_provisional_page_type_and_callback_fires_for_new_pages_only():
    ...
    assert [p.url for p in seen] == urls(
        manifest
    )  # one callback per manifest entry, none for variants


def test_multiple_start_urls_and_run_id():
    pages = {"https://e.com/": ([], "home"), "https://e.com/login": ([], "login")}
    manifest = DiscoveryCrawler(FakeDriver(pages)).discover(
        ["https://e.com/", "https://e.com/login"], run_id="run-1"
    )
...
def test_manifest_is_written_on_success_and_on_interruption(tmp_path):
    ...
    ok = DiscoveryCrawler(FakeDriver(pages)).discover(
        ["https://e.com/"], manifest_path=tmp_path / "ok.json"
    )
